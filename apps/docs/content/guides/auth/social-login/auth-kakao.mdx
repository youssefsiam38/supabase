...
<Partial path="social_provider_setup" substitutions={{ provider: "Kakao" }} />

Add the callback URL on Kakao by going to `Product settings` > `Kakao Login` > `Redirect URI`.
...
<Partial path="social_provider_settings_supabase" substitutions={{ provider: "Kakao" }} />
